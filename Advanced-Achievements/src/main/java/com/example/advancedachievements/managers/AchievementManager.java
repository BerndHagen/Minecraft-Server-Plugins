...
public class AchievementManager {
    ...
    private Achievement createAchievementFromConfig(String id, ConfigurationSection section) {
        ...
        String category = section.getString("category", plugin.getConfigManager().getDefaultCategory());
        ...
        Map<String, Object> rewardData = new HashMap<>();        int points = section.getInt("points", plugin.getConfigManager().getDefaultPoints());
        boolean hidden = section.getBoolean("hidden", false);
        List<String> prerequisites = section.getStringList("prerequisites");
        return new Achievement(id, title, description, category, icon, taskType, taskTarget, requiredAmount, rewards, rewardData, points, hidden, prerequisites);
    }
    ...
    private void createDefaultAchievements() {
        List<Achievement> defaultAchievements = Arrays.asList(
            new Achievement("first_blocks", "First Steps", "Break your first 10 blocks", 
                          "Building", Material.COBBLESTONE, TaskType.BLOCK_BREAK, "ANY", 10, 
                          Arrays.asList("ITEM:BREAD:5", "XP:100"), new HashMap<>(), 10, false, new ArrayList<>()),
            new Achievement("wood_collector", "Wood Collector", "Collect 64 wood logs", 
                          "Building", Material.OAK_LOG, TaskType.ITEM_PICKUP, "LOG", 64, 
                          Arrays.asList("ITEM:IRON_AXE:1", "XP:200"), new HashMap<>(), 20, false, new ArrayList<>()),
            new Achievement("monster_hunter", "Monster Hunter", "Kill 25 hostile mobs", 
                          "Combat", Material.IRON_SWORD, TaskType.MOB_KILL, "HOSTILE", 25, 
                          Arrays.asList("ITEM:GOLDEN_APPLE:3", "XP:500"), new HashMap<>(), 30, false, new ArrayList<>()),
            new Achievement("master_crafter", "Master Crafter", "Craft 100 items", 
                          "Crafting", Material.CRAFTING_TABLE, TaskType.ITEM_CRAFT, "ANY", 100, 
                          Arrays.asList("ITEM:DIAMOND:5", "XP:1000"), new HashMap<>(), 50, false, new ArrayList<>())
        );
    ...
    public Achievement createAchievement(String id, String title, String description, String category,
        ...
        Achievement achievement = new Achievement(id, title, description, category, icon,
                                                taskType, taskTarget, requiredAmount, rewards, 
                                                rewardData, points, hidden, prerequisites);
        achievements.put(id, achievement);
        saveAchievementToFile(achievement);
        plugin.getDatabaseManager().saveAchievement(achievement);
        return achievement;
    ...
    private void saveAchievementToFile(Achievement achievement) {
        ...
        achievementsConfig.set(path + ".category", achievement.getCategory());
        ...
        achievementsConfig.set(path + ".points", achievement.getPoints());
    ...
    public Set<String> getCategories() {
        ...
        for (Achievement achievement : achievements.values()) {
            categories.add(achievement.getCategory());
        }
        return categories;
    }
