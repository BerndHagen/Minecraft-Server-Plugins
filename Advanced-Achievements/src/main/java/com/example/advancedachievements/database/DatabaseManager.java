...
public class DatabaseManager {
    ...
    public CompletableFuture<Void> saveAchievement(Achievement achievement) {
        return CompletableFuture.runAsync(() -> {            String sql = "INSERT OR REPLACE INTO achievements (id, title, description, category, icon, " +
                    "task_type, task_target, required_amount, rewards, reward_data, points, hidden, prerequisites) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            try (PreparedStatement stmt = connection.prepareStatement(sql)) {
                ...
                stmt.setString(5, achievement.getIcon().name());
                stmt.setString(6, achievement.getTaskType().getId());
                stmt.setString(7, achievement.getTaskTarget());
                stmt.setInt(8, achievement.getRequiredAmount());
                stmt.setString(9, String.join(",", achievement.getRewards()));
                stmt.setString(10, serializeMap(achievement.getRewardData()));
                stmt.setInt(11, achievement.getPoints());
                stmt.setBoolean(12, achievement.isHidden());
                stmt.setString(13, String.join(",", achievement.getPrerequisites()));
                stmt.executeUpdate();
            } catch (SQLException e) {
    ...
    public CompletableFuture<List<Achievement>> loadAchievements() {
        return CompletableFuture.supplyAsync(() -> {
            ...
            try (Statement stmt = connection.createStatement();
                ...
                while (rs.next()) {                    Achievement achievement = new Achievement(
                            rs.getString("id"),
                            rs.getString("title"),
